...
const cachelog = winston.loggers.get('cachelog');
...
const startInvalidationWatcher = (interval) => {
  invalidationWatcherId = setInterval(() => {
    cachelog.info('Invalidating old entries');
  }, interval * 1000);
};
...
const stopInvalidationWatcher = () => {
  cachelog.info('Stopping invalidation watcher');
...
const add = (key, data, tags = ['global'], ttl = 100) => {
  return new Promise((resolve) => {
    cachelog.verbose(`Adding '${key}' to cache`); // TODO: Performance critical
...
const get = (key) => {
  return new Promise((resolve, reject) => {
    if (key in cacheStore) {
      cachelog.verbose(`Fetching data from key '${key}'`);
      return resolve(cacheStore[key].data);
    }

    cachelog.warn(`Cache miss for key '${key}'`);
